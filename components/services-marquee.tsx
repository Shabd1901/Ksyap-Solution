...
const ServicesMarquee = () => {
  return (
    <section id="services-marquee" className="py-20 px-4 bg-white overflow-hidden relative">
      {/* Background accents */}
      ...
      <div className="max-w-7xl mx-auto text-center mb-12 relative z-10">
        ...
        <p className="text-gray-700 text-xl">Innovative digital solutions built for business success</p>
      </div>
          ...
          {[...services, ...services].map((service, index) => {
            ...
            return (
              <motion.div
                ...
                className="group/tile relative min-w-[300px] max-w-xs bg-white border border-gray-200 shadow-lg rounded-xl p-6 transition-all duration-500 overflow-hidden hover:z-20 cursor-pointer flex flex-col justify-between"
              >
                {/* Icon */}
                <div
                  className={`p-4 rounded-full bg-gradient-to-r ${service.gradient} bg-opacity-10 border border-gray-200 mb-4 w-fit mx-auto`}
                >
                  <Icon className="h-8 w-8 text-gray-800" />
                </div>

                {/* Title */}
                <h3 className="text-xl font-semibold bg-gradient-to-r from-cyan-600 to-blue-700 bg-clip-text text-transparent text-center">
                  {service.title}
                </h3>
